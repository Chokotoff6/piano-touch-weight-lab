...
export type FaqPage = "home" | "saisie" | "resultats" | "comparer";

/** Titre de la fenêtre flottante FAQ selon la page courante. */
export function faqTitle(page: FaqPage, en: boolean): string {
  if (page === "saisie") return en ? "FAQ - Inputs" : "FAQ - Saisie";
  if (page === "resultats") return en ? "FAQ - Results" : "FAQ - Résultats";
  if (page === "comparer") return en ? "FAQ - Compare" : "FAQ - Comparer";
  return en ? "FAQ - Home" : "FAQ - Accueil";
}
  ...
  return (
    <Accordion type="single" collapsible className="w-full">
          <AccordionItem value="rgpd">
            ...
            <AccordionContent>
              <p>
                KeyWeight respecte strictement votre vie privée. L'application
                collecte uniquement les données techniques liées au piano (modèle,
                année, numéro de série, mesures de poids). Aucune donnée nominative
                ou personnelle n'est stockée ni partagée à votre insu. Les pesées
                partagées sur le Cloud collaboratif sont totalement anonymisées et
                servent uniquement à enrichir l'outil d'analyse comparative de la
                communauté. Vous restez maître de vos fichiers et de vos partages à
                chaque instant.
              </p>
            </AccordionContent>
          </AccordionItem>
